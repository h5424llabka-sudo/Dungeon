...
const SCROLLS = [
  ...
  { id: 's_remove',    name: '呪い解除の巻物', unknownName: '赤い巻物',  weight: 10, effect: { type: 'remove_curse'        }, desc: '呪われた装備を解除する。' },
  { id: 's_enhance',   name: '強化の巻物',   unknownName: '金の巻物',    weight: 8,  effect: { type: 'enhance',  amount: 3 }, desc: '装備中の武器か防具を+3強化する。' },
  { id: 's_warp_all',  name: '混乱の巻物',   unknownName: '青い巻物',    weight: 15, effect: { type: 'warp_enemy'         }, desc: '全ての敵をフロア内にワープさせる。' },
  { id: 's_monster',   name: '召喚の巻物',   unknownName: '黒い巻物',    weight: 10, effect: { type: 'summon'              }, desc: '周囲に敵を3体召喚する。' },
];

// ── 杖 ───────────────────────────────────────────────
const STAFFS = [
  { id: 'st_stun',    name: '痺れの杖',   unknownName: '青い杖',   charges: [3,6], effect: { type: 'stun',     duration: 2 }, desc: '対象を2ターン行動不能にする。' },
  { id: 'st_sleep',   name: '眠りの杖',   unknownName: '白い杖',   charges: [3,6], effect: { type: 'sleep',    duration: 4 }, desc: '対象を4ターン眠らせる。' },
  { id: 'st_warp',    name: '転送の杖',   unknownName: '緑の杖',   charges: [3,5], effect: { type: 'warp'                 }, desc: '対象をランダム位置にワープさせる。' },
  { id: 'st_push',    name: '吹き飛ばしの杖', unknownName: '短い杖', charges:[3,6], effect: { type: 'push',   distance: 5 }, desc: '対象を5マス後ろに吹き飛ばす。' },
  { id: 'st_clone',   name: '分身の杖',   unknownName: '赤い杖',   charges: [2,4], effect: { type: 'clone'               }, desc: '対象のコピーを生成する。敵に使うと危険。' },
  { id: 'st_heal',    name: '回復の杖',   unknownName: '金の杖',   charges: [3,5], effect: { type: 'heal',    amount: 50  }, desc: '対象のHPを50回復する。' },
];

// ── 食料 ─────────────────────────────────────────────
const FOODS = [
  { id: 'f_onigiri',   name: 'おにぎり',     hunger: 50, weight: 60, desc: '定番の携帯食料。満腹度+50。' },
  { id: 'f_bread',     name: '堅パン',        hunger: 30, weight: 80, desc: '硬くて食べにくいが長持ち。' },
  { id: 'f_meat',      name: '焼き肉',        hunger: 70, weight: 20, desc: '香ばしい肉。満腹度+70。' },
  { id: 'f_candy',     name: '回復飴',        hunger: 20, weight: 30, hp: 10, desc: '甘い飴。満腹度+20・HP+10。' },
];
...
export function createRandomItem(floor) {
  ...
  let type;
  if      (typeRoll < 0.08) type = ITEM_TYPE.WEAPON;
  else if (typeRoll < 0.16) type = ITEM_TYPE.ARMOR;
  else if (typeRoll < 0.35) type = ITEM_TYPE.GRASS;
  else if (typeRoll < 0.52) type = ITEM_TYPE.SCROLL;
  else if (typeRoll < 0.65) type = ITEM_TYPE.STAFF;
  else if (typeRoll < 0.80) type = ITEM_TYPE.FOOD;
  else if (typeRoll < 0.90) type = ITEM_TYPE.POT;
 ...
 * 杖を振る
 */
export function useStaff(staff, target, player, gameCtx) {
  if (staff.charges <= 0) return '杖の回数が切れている。';
  identifyItem(staff.id);
  staff.identified = true;
  staff.charges--;

  const eff = staff.effect;
  if (!target) return '空振りした。';

  switch (eff.type) {
    case 'stun':   target.addStatus('stun',  eff.duration); return `${target.name}を麻痺させた！`;
    case 'sleep':  target.addStatus('sleep', eff.duration); return `${target.name}を眠らせた！`;
    case 'warp': {
      const pos = gameCtx.getRandomFloor();
      target.x = pos.x; target.y = pos.y;
      return `${target.name}をワープさせた！`;
    }
    case 'push': {
      // 直線上にdistance分吹き飛ばす（簡易）
      return `${target.name}を吹き飛ばした！`;
    }
    case 'heal': {
      target.hp = Math.min(target.hpMax, target.hp + (eff.amount || 50));
      return `${target.name}のHPが回復した。`;
    }
    default: return '何かが起きた。';
  }
}
